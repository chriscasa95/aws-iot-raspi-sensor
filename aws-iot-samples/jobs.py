...
cmdUtils = command_line_utils.CommandLineUtils(
    "Jobs - Recieve and execute operations on the device."
)
...
cmdUtils.register_command(
    "thing_name", "<str>", "The name assigned to your IoT Thing", required=True
)
cmdUtils.register_command(
    "job_time",
    "<int>",
...
def job_thread_fn(job_id, job_document):
    try:
        ...
        publish_future = jobs_client.publish_update_job_execution(
            request, mqtt.QoS.AT_LEAST_ONCE
        )
...
if __name__ == "__main__":
    ...
    try:
        ...
        update_subscription_request = iotjobs.UpdateJobExecutionSubscriptionRequest(
            thing_name=jobs_thing_name, job_id="+"
        )
