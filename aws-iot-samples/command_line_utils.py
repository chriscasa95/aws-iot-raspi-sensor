...
class CommandLineUtils:
    ...
    def get_args(self):
        ...
        if self.parsed_commands.verbosity:
            io.init_logging(
                getattr(io.LogLevel, self.parsed_commands.verbosity), "stderr"
            )
        return self.parsed_commands
    ...
    def update_command(
        self,
        command_name,
        new_example_input=None,
        new_help_output=None,
        new_required=None,
        new_type=None,
        new_default=None,
        new_action=None,
    ...
    def build_pkcs11_mqtt_connection(
    ...
    def build_websocket_mqtt_connection(
        self, on_connection_interrupted, on_connection_resumed
    ):
    ...
    def build_mqtt_connection(self, on_connection_interrupted, on_connection_resumed):
        if self.get_command(self.m_cmd_signing_region) is not None:
            return self.build_websocket_mqtt_connection(
                on_connection_interrupted, on_connection_resumed
            )
        else:
            return self.build_direct_mqtt_connection(
                on_connection_interrupted, on_connection_resumed
            )
