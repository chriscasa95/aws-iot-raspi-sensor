...
export const availableDevices = ['C3', 'HTWG', 'Raspi']
...
export async function transformDataPoints(data, limit, interpolate) {
    ...
    .reduce((acc, item) => {
        if (item.timestamp) {
            acc[0].push([new Date(item.timestamp * 1000).getTime(), isNaN(item.temperature) ? null : item.temperature]);
            acc[1].push([new Date(item.timestamp * 1000).getTime(), isNaN(item.humidity) ? null : item.humidity]);
            acc[2].push([new Date(item.timestamp * 1000).getTime(), isNaN(item.pm2_5) ? null : item.pm2_5]);
            acc[3].push([new Date(item.timestamp * 1000).getTime(), isNaN(item.pm10) ? null : item.pm10]);
        }
        return acc;
